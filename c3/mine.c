/*
 * A simple CPU miner for ACM Coin, a fake cryptocurrency
 * SHA256 hash implementation courtesy of BareRose on GitHub
 * 
 * You can try tweaking MAX_BLOCKS to try and mine more blocks
 * but if your program goes over 10 minutes, you won't get anything
*/
#define MAX_BLOCKS 1 // How many blocks to mine
...
#define DIFFICULTY 3 // How many zeroes to expect

// Normally we would include transaction data
struct block {
    int index;
...
// True if first (diff) bytes of hash are zero
int validate(unsigned char hash[32]) {
...
int main(void) {
    ...
    struct block cur;
    cur.index = 0;
    memset(cur.prev_hash, 0, 32);

    unsigned char hash[32] = {0};

    while(cur.index < MAX_BLOCKS) {  

        // Try increasing nonce until hash valid
        cur.nonce = 0;
        do {
            sha256(hash, (const unsigned char*) &cur, sizeof(struct block));
            cur.nonce++;
        } while(!validate(hash));

        // Print out block data
        printf("Mined block %d: ", cur.index);
        for(int i = 0; i < 32; i++) {
            if(i == DIFFICULTY) putchar(' ');
            printf("%02x", hash[i]);
        }
        printf(" nonce %lld\n", cur.nonce);
